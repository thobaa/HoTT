...
(** We define these dually to reflective subuniverses.  However, as we will see, it turns out that there are many fewer examples.  Thus, since there are not many applications, we don't bother introducing modules to make definitions more universe polymorphic. *)
...
Section CoreflectiveSubuniverse.
  ...
  (** The coreflector preserves hprops. *)
  ...
  Proof.
    ...
    intros x; unfold compose; apply p.
  Defined.

  (** We claim that [F] is completely determined by the image of [Unit], which by the previous lemma is an hprop. Specifically, we claim that [X] lies in [F] exactly when [X -> F Unit]. *)
  ...
  Proof.
    ...
    - intros f.
      ...
      + intros x; unfold F_functor, compose.
        exact (F_corec_beta (F_inF F Unit) (const x) (f x)).
  Defined.

End CoreflectiveSubuniverse.

(** Conversely, we will now show that for any hprop [U], the types [X] such that [X -> U] are a coreflective subuniverse. *)
  ...
  Proof.
    ...
    - intros ux; simpl in *.
      ...
      + apply ap10, ap, path_arrow; intros u'.
        apply path_prod; [ apply ap | ]; apply path_ishprop.
